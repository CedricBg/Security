...
AS
Begin
	Declare @IdRetour Table(
	Id int
	);
	
	DECLARE @TableInt TABLE(
		Id int
	);
	SET @IdRetour.Id = 0
	...
	or ((SELECT TOP 1 DepartureTime FROM Start_end_Time WHERE IdEmployee = @IdEmployee and IdCustomer = @IdCustomer ORDER BY IdStart DESC ) is Null 
	And (SELECT TOP 1 ArrivingTime FROM Start_end_Time WHERE IdEmployee = @IdEmployee and IdCustomer = @IdCustomer ORDER BY IdStart DESC  ) IS NULL)
	Begin
	INSERT INTO Start_end_Time (IdCustomer,IdEmployee,ArrivingTime) OUTPUT inserted.IdStart into @TableInt Values(@IdCustomer, @IdEmployee, GETDATE())
	Select Id from @TableInt
	end
	else
		begin
			Select Id from @IdRetour
		end
