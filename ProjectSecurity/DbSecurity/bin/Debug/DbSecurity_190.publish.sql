...
Begin
	...
	SET @Time.Arriving  =  (SELECT TOP 1 ArrivingTime FROM Start_end_Time WHERE IdEmployee = @IdEmployee and IdCustomer = @IdCustomer ORDER BY IdStart DESC )
	SET @Time.Departure  = (SELECT TOP 1 DepartureTime FROM Start_end_Time WHERE IdEmployee = @IdEmployee and IdCustomer = @IdCustomer ORDER BY IdStart DESC )
	
	if (((SELECT Departure from @Time)  IS NULL 
	And (SELECT Arriving  from @Time)  IS NULL)

	or ((SELECT Departure from @Time)  IS NULL
	And (SELECT Arriving  from @Time)  IS NULL))
	Begin
